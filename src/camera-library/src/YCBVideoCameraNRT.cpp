...
YcbVideoCameraNrt::YcbVideoCameraNrt(const std::string& path, const std::size_t& width, const std::size_t& height, const std::string& config_context)
{
    ...
    // std::cout << log_ID_ + "::ctor. Loading rgb images..." << std::flush;
    // bool is_file = true;
    // std::size_t num_frames = 0;
    // while (is_file)
    // {
    //     cv::Mat image_in = cv::imread(path_rgb_images + composeFileName(num_frames, number_of_digits) + ".jpg", cv::IMREAD_COLOR);
    //     if (is_file = (!image_in.empty()))
    //     {
    //         rgb_images_.push_back(image_in);
    //         num_frames++;
    //     }
    // }
    // std::cout << "done (" << num_frames << " frames loaded)." << std::endl;

    // /* Load all the depth images. */
    // std::cout << log_ID_ + "::ctor. Loading depth images..." << std::flush;
    // for (std::size_t i = 0; i < num_frames; i++)
    // {
    //     std::FILE* in;
    //     std::string file_name = path_depth_images + composeFileName(i, number_of_digits) + ".float";
    ...
    //     float_image = Map<MatrixXf>(float_image_raw, dims[1], dims[0]);
    //     depth_images_.push_back(float_image);
...
std::pair<bool, Eigen::MatrixXf> YcbVideoCameraNrt::getDepthImage(const bool& blocking)
{
    std::FILE* in;
