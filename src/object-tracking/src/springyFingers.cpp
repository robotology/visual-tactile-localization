...
{
    port_arm=new iCub::perception::Port;
    port_analogs=new iCub::perception::Port;
    configured=false;
}
...
bool SpringyFingersModel::fromProperty(const Property &options)
{
    ...
    port_analogs->open("/"+name+"/"+part_analog+"/analog:i");
    string analogPortName("/"+robot+"/"+part_analog+"/analog:o");
    if (!Network::connect(analogPortName,port_analogs->getName(),carrier))
    {
    ...
    port_arm->open("/"+name+"/"+part_motor+"/state:i");
    string armPortName("/"+robot+"/"+part_motor+"/state:o");
    if (!Network::connect(armPortName,port_arm->getName(),carrier))
    {
        printMessage(log::error,1,"unable to connect to %s",armPortName.c_str());
        close();
        return false;
    }
    ...
    Property thumb_flex( "(name In_0) (index 10)" );
    Property index_flex( "(name In_0) (index 12)" );
    Property middle_flex("(name In_0) (index 14)" );
    Property ring_flex(  "(name In_0) (index 15)" );
    Property little_flex("(name In_0) (index 15)" );

    void *pPortArm=static_cast<void*>(port_arm);
    bool sensors_ok=true;
    sensors_ok&=sensEncs[0].configure(pPortArm,thumb_flex);
    sensors_ok&=sensEncs[1].configure(pPortArm,index_flex);
    sensors_ok&=sensEncs[2].configure(pPortArm,middle_flex);
    sensors_ok&=sensEncs[3].configure(pPortArm,ring_flex);
    sensors_ok&=sensEncs[4].configure(pPortArm,little_flex);
    if (!sensors_ok)
    {
    ...
    void *pPortAnalogs=static_cast<void*>(port_analogs);
    sensors_ok&=sensPort[0].configure(pPortAnalogs,thumb_mp);
    sensors_ok&=sensPort[1].configure(pPortAnalogs,thumb_ip);
    sensors_ok&=sensPort[2].configure(pPortAnalogs,index_mp);
    sensors_ok&=sensPort[3].configure(pPortAnalogs,index_ip);
    sensors_ok&=sensPort[4].configure(pPortAnalogs,middle_mp);
    sensors_ok&=sensPort[5].configure(pPortAnalogs,middle_ip);
    sensors_ok&=sensPort[6].configure(pPortAnalogs,ring_mp);
    sensors_ok&=sensPort[7].configure(pPortAnalogs,ring_pip);
    sensors_ok&=sensPort[8].configure(pPortAnalogs,ring_dip);
    sensors_ok&=sensPort[9].configure(pPortAnalogs,little_mp);
    sensors_ok&=sensPort[10].configure(pPortAnalogs,little_pip);
    sensors_ok&=sensPort[11].configure(pPortAnalogs,little_dip);
