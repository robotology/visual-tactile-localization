...
bool SFM::configure(ResourceFinder &rf)
{
    ...
    if (rf.check("use_elas", Value(true)).asBool())
        stereo->initELAS(rf);
    ...
    {
        cout << "Error while loading Extrinsic parameters!" << endl;
        return false;
    }
...
{
    ImageOf<PixelRgb> *yarp_imgL=leftImgPort.read(do_block);
    ImageOf<PixelRgb> *yarp_imgR=rightImgPort.read(do_block);
...
bool SFM::loadExtrinsics(yarp::os::ResourceFinder& rf, Mat& Ro, Mat& To, yarp::sig::Vector& eyes)
{
    ...
    cout<<"read relative eyes orientation = ("<<Ro<<")"<<endl;
    cout<<"read relative eyes position = ("<<To<<")"<<endl;
        ...
        Mat &DistR)
{
    ...
    cout << "load left instrinsics" << endl;
    cout << "fx: " << fx << endl;
    cout << "fy: " << fy << endl;
    cout << "cx: " << cx << endl;
    cout << "cy: " << cy << endl;
    ...
    cout << "load right instrinsics" << endl;
    cout << "fx: " << fx << endl;
    cout << "fy: " << fy << endl;
    cout << "cx: " << cx << endl;
    cout << "cy: " << cy << endl;
...
std::tuple<bool, Eigen::MatrixXd, Eigen::VectorXi> SFM::get3DPoints(const std::vector<std::pair<int, int>> & u_v_coordinates, const bool do_block, const float left_z_threshold)
{
    ...
    Eigen::VectorXi valid_points(u_v_coordinates.size());
    ...
    for (int i = 0; i < u_v_coordinates.size(); i++)
    {
        valid_points(i) = 0;

        const int& u_tmp = u_v_coordinates[i].first;
        const int& v_tmp = u_v_coordinates[i].second;

        float usign=Mapper.ptr<float>(v_tmp)[2 * u_tmp];
        float vsign=Mapper.ptr<float>(v_tmp)[2 * u_tmp + 1];

        int u=cvRound(usign);
        int v=cvRound(vsign);

        if ((u<0) || (u>=disp16m.cols) || (v<0) || (v>=disp16m.rows))
        {
            // skip this point as it is invalid
            continue;
        }
        ...
        points_all(0, i) =(float)((usign+1)*Q.at<double>(0,0)+Q.at<double>(0,3));
        points_all(1, i) =(float)((vsign+1)*Q.at<double>(1,1)+Q.at<double>(1,3));
        points_all(2, i) =(float)Q.at<double>(2,3);

        // scale z coordinate
        points_all(2, i) = float(points_all(2, i)) / w;

        // discard points far more than left_z_threshold meters or with not valid disparity (<0)
        if ((points_all(2, i) > left_z_threshold) || (points_all(2, i) < 0.0f))
        {
            continue;
        }

        // scale also x and y coordinate once it is known that the point is good
        points_all.col(i).head<2>() /= w;

        valid_points(i) = 1;
    }

    // number of valid points
    std::size_t num_valid_points = valid_points.sum();
    ...
    for (int i = 0, j = 0; i < points_all.cols(); i++)
    {
        if (valid_points(i) == 1)
        {
            points_all.col(i).swap(points.col(j));
            j++;
        }
    }

    // express w.r.t. robot root frame
    const Mat& RLrect=this->stereo->getRLrect().t();
    Mat Tfake=Mat::zeros(0,3,CV_64F);
    Mat Hrect=buildRotTras(RLrect,Tfake);

    Eigen::Map<Eigen::Matrix<double, 4, 4, Eigen::RowMajor>> HL_root_eigen(HL_root.ptr<double>(), HL_root.rows, HL_root.cols);
    Eigen::Map<Eigen::Matrix<double, 4, 4, Eigen::RowMajor>> Hrect_eigen(Hrect.ptr<double>(), Hrect.rows, Hrect.cols);

    Eigen::Transform<double, 3, Eigen::Projective> H(HL_root_eigen * Hrect_eigen);
    points = (H * points.colwise().homogeneous()).colwise().hnormalized();
...
bool SFM::respond(const Bottle& command, Bottle& reply)
{
    ...
    else if (command.get(0).asString()=="Point" || command.get(0).asString()=="Left" )
    {
        ...
        Point3f point = this->get3DPoint(u,v);
    ...
    else if (command.get(0).asString()=="Right")
    {
        int u = command.get(1).asInt();
        int v = command.get(2).asInt();
        Point3f point = this->get3DPoint(u,v,"RIGHT");
        reply.addDouble(point.x);
        reply.addDouble(point.y);
        reply.addDouble(point.z);
    }
    else if (command.get(0).asString()=="Root")
    {
        int u = command.get(1).asInt();
        int v = command.get(2).asInt();
        Point3f point = this->get3DPoint(u,v,"ROOT");
        reply.addDouble(point.x);
        reply.addDouble(point.y);
        reply.addDouble(point.z);
    }
    ...
    else if (command.get(0).asString()=="Flood3D")
    {
        ...
        Point3f p=get3DPoint(seed.x,seed.y,"ROOT");
        if (cv::norm(p)>0.0)
        {
            reply.addInt(seed.x);
            reply.addInt(seed.y);
