...
{  
    ...
    cout<<"Result with Gaussian: "<<result4.toString().c_str()<<endl;
...
bool UnscentedParticleFilter::configure(ResourceFinder &rf, const int &i)
{
    this->rf=&rf;
    stringstream ss;
    ss << i;
    ...
    modelFileName=rf.find("modelFile"+istring).asString().c_str();
    ...
    down=rf.find("downsampling").asInt();
    if (rf.find("downsampling").isNull())
        down=1;
